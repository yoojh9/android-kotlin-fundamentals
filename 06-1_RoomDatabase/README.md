# 06-1 Create a Room database
 ...
 <image src="./images/room_database.png" width="50%" height="50%"/>
 ...
 **The sleepquality and sleeptracker package**에는 fragment, view model, view model factory가 각각의 스크린마다 존재한다
 ...
 #### 4) android Test 폴더 (SleepDatabaseTest.kt)에서 database가 의도한대로 작동는지 테스트 해 볼 수 있다

<br><br>

## 2. Create the SleepNight entity
 - **entity**는 데이터베이스에 저장할 개체나 개념 및 속성을 나타낸다. entity 클래스는 테이블을 의미하고, 엔티티 클래스의 각 인스턴스는 테이블의 row를 나타낸다. 각 프로퍼티는 컬럼을 의미한다
 - **query**는 하나의 데이터베이스 테이블 또는 결합된 테이블에서 데이터를 요청하거나 데이터에 대한 처리를 요청한다. 일반적인 query는 entity를 가져오고 추가하고 수정한다
 
 각 엔티티는 어노테이션이 있는 data class로 정의하고 DAO(data access object)와 상호작용 해야한다. Room은 어노테이션이붙은 클래스를 사용하여 테이블을 생성하고 데이터베이스에서 작동하는 쿼리를 이용한다
 
 ### Step 1: Create the SleepNight entity
  이 작업에서는 night of sleep을 어노테이션이 달린 data class로 정의한다. start time, end time, quality rating을 기록해야 하며 night를 구별할 유니크한 ID도 필요하다
  ...
  #### 5) 남아있는 프로퍼티에 @ColumnInfo 어노테이션을 추가한다. 파라미터로 name 프로퍼티를 사용하여 이름을 커스텀할 수 있다
...
## 3. Create the DAO
 ...
 ### Step 1: Create the SleepDatabase DAO
  ...
  #### 3) interface의 body 부분에 @Insert 어노테이션을 추가하고 @Insert 어노테이션 아래에 insert() 함수를 추가한다.
   ...
   - 코틀린 코드에서 insert() 함수를 호출하면 Room은 SQL 쿼리를 실행시켜 데이터베이스에 entity를 추가한다
   ...
   - 쿼리에서는 : 표기법을 사용하여 함수의 인자를 참조한다 
   
   <br>
   
   #### 6) @Query를 어노테이션으로 하는 clear() 함수를 만든다. 이 함수는 daily_sleep_quality_table의 모든 데이터를 DELETE 한다
   ...
   #### 7) getTonight() 함수에 @Query 어노테이션을 추가하여 nullable한 SleepNight을 리턴하는 함수를 만든다. 오늘밤의 데이터를 얻기 위해서 nightId 키를 오름차순으로 정렬한 값 중 LIMIT 1을 사용하여 하나의 요소를 전달한다
   ...
   #### 8) @Query 어노테이션을 getAllNights() 함수에 추가한다
   - SQLite 쿼리가 daily_sleep_quality_table의 모든 컬럼을 내림차순으로 반환하도록 한다
   - getAllNights()가 SleepNight 엔티티의 리스트를 LiveData로 리턴하도록 한다.
...
## 4. Create and test a Room database
 ...
 @Database 어노테이션이 달린 추상 데이터베이스 홀더 클래스를 만들어야 한다. 이 클래스에는 데이터베이스 인스턴스가 존재하지 않는 경우 데이터베이스의 인스턴스를 생성하거나 데이터베이스의 인스턴스가 이미 존재할 경우 데이터베이스에 대한 참조를 리턴하는 하나의 메소드가 있다
 ...
 Room 데이터베이스를 얻는 것은 다소 복잡하므로 코드를 시작하기 전에 일반적인 프로세스는 다음과 같다
   ...
   - @Database 어노테이션을 추가하고 인자로 데이터베이스의 엔티티를 선언하고 버전 번호를 설정한다
   - companion object에서 SleepDatabaseDao를 리턴하는 추상 메소드 및 속성을 정의한다.
 ...
 ### Step 1: Create the database
  ...
  #### 2) 파일에 @Database 어노테이션을 추가하고 RoomDatabase를 상속한 abstract 클래스인 SleepDatabase 클래스를 생성한다
  ...
  #### 3) 위의 코드를 실행할 경우 entities와 version 파라미터가 없다는 error를 보게 된다. Room이 데이터베이스를 만드려면 @Database 어노테이션에 몇가지 argument가 필요하다
   ...
   - entities의 목록으로 SleepNight를 전달한다
  ...
  #### 4) database는 DAO에 대해 알기를 원하므로 class의 body 안에 SleepDatabaseDao 값을 리턴하는 abstract val을 선언한다. 여러개의 DAO를 가질 수도 있다
  ...
  #### 6) companion object 내에 database를 위한 private nullable 변수로 INSTANCE를 선언하고 null로 초기화 한다. INSTANCE 변수는 일단 데이터베이스가 생성되면 데이터베이스의 참조를 유지한다. 이렇게 하면 데이터베이스의 connection을 반복적으로 열지 않아도 된다.
  
   - INSTANCE에 @Volatile 어노테이션을 추가한다. volatile 변수의 값은 절대 캐시되지 않으며 모든 읽고 쓰는 작업은 main memory에서 이루어진다. 이를 통해 INSTANCE 값을 항상 최신으로 유지하고 모든 쓰레드의 실행을 동일하게 유지한다. 
  ...
  #### 8) getInstance() 메소드에 synchronized{} 블럭을 추가하고 context에 액세스 할 수 있도록 this를 전달한다. 
  ...
  #### 9) synchronized 블럭에서 INSTANCE의 현재 값을 instance 지역 변수로 복사한다. 이는 로컬 변수에서만 사용할 수 있는 스마트 캐스트를 활용하기 위한 것이다.
  ...
  #### 12) 만약 instance가 null이면 database builder를 사용하여 database를 얻어온다. if문의 body에서 Room.databaseBuilder를 호출하고 넘겨받은 context와 database 클래스, 그리고 database 이름인 sleep_history_database를 파라미터로 넘긴다.
   - 에러를 제거하기 위해 migration 전략과 build() 함수를 다음 단계에서 추가한
  ...
  #### 13) builder에 migration 전략을 추가한다. .fallbackToDestructiveMigration()을 사용한다.
   ...
   - migration object는 이전 스키마가 있는 모든 행을 가져와서 새 스키마의 행으로 변환하여 데이터가 손실되지 않도록 정의하는 객체이다
 ...
 #### 1) androidTest 폴더에서 SleepDatabaseTest 파일을 열어서 주석 처리 된 코드의 주석을 지우고 코드르 살펴보자
   ...
   - @Test 주석이 달린 테스트 메소드를 통해 create, insert, retrieve 테스트를 진행해 볼 수 있다
